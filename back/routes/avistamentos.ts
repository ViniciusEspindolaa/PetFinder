...
router.post("/", async (req, res) => {
  ...
  const { publicacaoId, usuarioId, observacoes, fotos_urls, latitude, longitude, endereco_texto } = valida.data
  ...
  try {
    ...
    // Enviar email de notificação para o dono da publicação
    try {
      await enviaEmailAvistamento(
        avistamento.publicacao.usuario.nome,
        avistamento.publicacao.usuario.email,
        avistamento
      );
    } catch (emailError) {
      console.error("Erro ao enviar email de avistamento:", emailError);
    }
